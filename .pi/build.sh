...
docker buildx build --platform linux/arm64 --tag seahorse --load --progress plain .
...
docker save --output ./.tmp/result-rootfs.tar seahorse
...
# Make the image file
fallocate -l 4G "images/seahorse.img"

# loop-mount the image file so it becomes a disk
export LOOP=$(sudo losetup --find --show images/seahorse.img)
